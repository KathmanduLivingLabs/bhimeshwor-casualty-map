...
function ready(error, districts_topo) {
  svg.append("g")
      .attr("class", "admindiv")
    .selectAll("path")
      .data(topojson.feature(districts_topo, districts_topo.objects.admindiv).features)
    .enter().append("path")
...
// Draw the state borders
  svg.append("path")
      .datum(topojson.mesh(districts_topo, districts_topo.objects.admindiv, function(a, b) { 
        return a !== b; }))
      .attr("class", "states")
      .attr("d", path)
